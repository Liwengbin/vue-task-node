<template>
  <g :transform="'translate('+node.positionX+','+node.positionY+')'" class="pane-cell pane-node">
    <g transform="scale(1,1)" class="pane-scalable">
      <foreignObject width="182" height="30" @click="[selectNode ? selectNode($event,node):selectNodes($event,node)]" @contextmenu.prevent="mouseMenu" :id="node.id">
...
<script>
  ...
  export default {
    ...
    methods:{
      mouseMenu:function (event) {//鼠标右击事件
